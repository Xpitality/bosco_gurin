...
class FcmSender
  ...
  def send(text_hash, id)
    ...
    text_hash.keys.each do |locale|
      @response = @fcm.send_with_notification_key("/topics/#{topic}#{locale.capitalize}", notification: {body: text_hash[locale], title: text_hash[locale].to_s.truncate_words(5)})
    end
  end
end